"""Regression tests for InMemoryTaskQueue._await_completion TTL semantics.

Verifies that:
- Double-await within TTL returns the same result (no KeyError on second call).
- After TTL expiry, _await_completion raises AwaitAlreadyConsumedError.
...
async def test_double_await_within_ttl_returns_same_result() -> None:
    """Second _await_completion within TTL must succeed, not raise."""
    ...
    r1 = await q._await_completion(task_id, timeout=1.0)
    r2 = await q._await_completion(task_id, timeout=1.0)
    assert r1.success is True
    assert r2.success is True


async def test_await_already_consumed_after_ttl_expiry() -> None:
    """After TTL expires, _await_completion raises AwaitAlreadyConsumedError."""
    ...
    # First call succeeds.
    await q._await_completion(task_id, timeout=1.0)
...
async def test_unknown_task_id_raises_key_error() -> None:
    ...
    with pytest.raises(KeyError):
        await q._await_completion("nonexistent-task", timeout=1.0)
...
async def test_pruned_ids_distinct_from_unknown() -> None:
    ...
    with pytest.raises(KeyError):
        await q._await_completion("ghost-task", timeout=1.0)
