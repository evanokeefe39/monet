...
- `src/monet/` — package source (src layout)
  ...
  - `orchestration/` — flat-DAG subgraphs and the compound default graph. Public surface: `RunState` (slim parent state for the compound graph, designed for `MyRunState(RunState)` extension), `build_planning_subgraph` / `build_execution_subgraph` (uncompiled `StateGraph`s for users composing their own pipelines), `build_default_graph` (composes planning + execution as nodes under `RunState`), `build_chat_graph` (slash-parse → inline triage → direct-LLM respond + planner/specialist dispatch; `planner_node` runs its own HITL approve/revise/reject loop via `interrupt()` with a form-schema payload, bounded by `PLAN_MAX_REVISIONS=3`). `triage_node` grounds the classifier in the live agent manifest (via `_known_agent_ids`) so specialist routing can't hallucinate agent ids — unknown specialists fall back to `planner`. Private state schemas and `RoutingSkeleton`/`RoutingNode` pydantic models in `_state.py`; `_invoke.py` (queue-only dispatch), `_signal_router.py`, `_validate.py`.
