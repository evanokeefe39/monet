...
- `src/monet/` — package source (src layout)
  ...
  - `server/` — `create_app()` (the FastAPI factory + ASGI lifespan), Aegra custom HTTP routes (`_aegra_routes.py` — lifespan registers reference agents via `monet.agents.register_reference_agents()`, heartbeats a `monolith-0` in-process worker, and spawns `run_worker` against the shared queue so S1 monolith serves handlers without a separate `monet worker` process), `_capabilities.py` (authoritative server-side `CapabilityIndex` populated by worker heartbeats, and the pydantic `Capability` wire model that validates charset / length at the boundary), langgraph config generation (`_langgraph_config.py`), `server_bootstrap.py` (0-arg Aegra wrappers for `build_chat_graph` and `build_default_graph`; promotes the `monet` logger namespace to INFO at boot — respecting `MONET_LOG_LEVEL` override — so `invoke_agent` dispatch, queue enqueue/claim/complete, worker execute, and server heartbeat records surface through Aegra's structlog pipeline). `_routes.py` exposes the unified `POST /api/v1/workers/{worker_id}/heartbeat` (registration = first heartbeat), `POST /api/v1/pools/{pool}/claim` (pool-scoped claim auth — worker must be heartbeating for that pool), `GET /api/v1/agents` (auth-gated; reads the `CapabilityIndex`), plus `GET /api/v1/artifacts/{id}` / `{id}/view`.
