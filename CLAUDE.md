...
- `src/monet/` — package source (src layout)
  ...
  - `cli/` — click commands: `_dev.py` (`monet dev` as a group with `down` subcommand + auto-teardown), `_run.py` (single generic streaming path for every entrypoint; form-schema interrupt prompting), `_runs.py` (list / pending / inspect / resume — `resume` renders the pending interrupt's form schema), `_chat.py` (thin Click entry — resolves thread, fetches slash commands + history, configures file logging to `./.cli-logs/chat.log` via `--log-file`, hands off to Textual), `chat/` subpackage: `_app.py` (Textual `ChatApp`: `RichLog` transcript with `_styled_line` role tag colours, toolbar, `RegistrySuggester` ghost-text, dropdown `OptionList` for slash completions with `tab` accept, `_PickerScreen`, `SidebarPanel` (ctrl+b) with agents / threads / artifacts / keys tabs, `MainMenuScreen` (ctrl+p) replacing the built-in command palette — submenus: options/theme, keyboard shortcuts, command library, about. TUI-local commands `/new` `/clear` `/threads` `/switch` `/agents` `/artifacts` `/runs` `/help` `/quit` `/exit`. HITL interrupts render as transcript text and the next user submission is parsed as the resume payload — no modal forms. `_format_form_prompt` + `_parse_text_reply` handle three shapes: approval (`approve | revise <feedback> | reject`), single-field, multi-field (one line per field). `_sidebar.py` — right-docked `SidebarPanel` with four `TabbedContent` tabs; min width `FLOOR_COLS=50`. `_menu.py` — `MainMenuScreen` + sub-screens (`OptionsScreen`, `KeyboardShortcutsScreen`, `CommandLibraryScreen`, `AboutScreen`). `_turn.py` — `run_turn` / `drain_stream` / `empty_stream` / `InterruptCoordinator`.), `_worker.py` (remote workers register via first heartbeat — no separate `monet register` command), `_server.py`, `_status.py`, `_render.py` (Click-based `render_event`, `render_interrupt_form` walking the form-schema vocabulary `text/textarea/radio/checkbox/select/int/bool/hidden` — used by `_run.py` and `_runs.py`), `_discovery.py`, `_setup.py`, `_db.py` (`monet db migrate | current | stamp | check`).
  - `client/` — `MonetClient` graph-agnostic client (`__init__.py`), core events (`_events.py`: `RunStarted`, `NodeUpdate`, `AgentProgress`, `SignalEmitted`, `Interrupt`, `RunComplete`, `RunFailed`, plus `RunSummary`, `RunDetail`, `PendingDecision`, `ChatSummary`, plus form-schema interrupt convention `Form` / `Field` / `FieldOption` TypedDicts), boundary errors (`_errors.py`: `MonetClientError` + `RunNotInterrupted`, `AlreadyResolved`, `AmbiguousInterrupt`, `InterruptTagMismatch`, `GraphNotInvocable`, `ServerUnreachable`, `ServerError`), wire helpers (`_wire.py`: `task_input`, `chat_input`, `stream_run`, `get_state_values`, `create_thread`, metadata keys, `_classify_transport_error` maps httpx errors to typed `MonetClientError` subclasses), run-state cache (`_run_state.py`: `{run_id: {graph_id: thread_id}}`).
  ...
  - `orchestration/` — flat-DAG subgraphs and the compound default graph. Public surface: `RunState` (slim parent state for the compound graph, designed for `MyRunState(RunState)` extension), `build_planning_subgraph` / `build_execution_subgraph` (uncompiled `StateGraph`s for users composing their own pipelines), `build_default_graph` (composes planning + execution as nodes under `RunState`), `build_chat_graph` (slash-parse → binary triage `{chat, plan}` → planner / questionnaire / approval three-node state machine, **then on approve mounts the same `build_execution_subgraph` as a node** + a small `execution_summary_node` that renders `wave_results` as a final assistant message — chat is plan-AND-execute end-to-end, not plan-only). Triage is information-only — returns route + confidence; it does not pick an agent. `planner_node` makes one agent call per visit and writes either `last_plan_output` or `pending_questions` into `ChatState`, plus the full `work_brief_pointer` (extracted from `result.artifacts` via `find_artifact("work_brief")`) and `routing_skeleton` so the execution subgraph picks them up by name-matching. `questionnaire_node` renders `pending_questions` as one `text` field per question; the TUI parses one line per field, with `skip` (or empty) dropping that answer. `approval_node` interrupts with approve/revise/reject — revise writes `plan_feedback` and routes back to planner; approve routes to the execution subgraph; reject terminates. Bounds: `PLAN_MAX_REVISIONS=3`, `MAX_FOLLOWUP_ATTEMPTS=1` (the agent is force-planned on the next call when exceeded). The reference planner agent emits either `{kind:"plan"}` with a WorkBrief or `{kind:"questions"}` + signal `NEEDS_CLARIFICATION`; orchestrator routes on the signal, not prose. Planner renders its prompt roster from an `agent_roster` context entry (shape `{"type": "agent_roster", "agents": [...]}`) injected by `_invoke_planner` from the server-side `CapabilityIndex` — ADR-004 — so split-fleet planners see every pool's capabilities without the agent speaking HTTP itself; unit tests / library callers without orchestration fall back to the worker's local registry. Private state schemas and `RoutingSkeleton`/`RoutingNode` pydantic models in `_state.py`; `_invoke.py` (queue-only dispatch), `_signal_router.py`, `_validate.py`.
  - `server/` — `create_app()` (the FastAPI factory + ASGI lifespan), Aegra custom HTTP routes (`_aegra_routes.py` — lifespan calls `bootstrap_server()` as the single queue-creation site, registers reference agents via `monet.agents.register_reference_agents()`, heartbeats a `monolith-0` in-process worker, and spawns `run_worker` against the shared queue so S1 monolith serves handlers without a separate `monet worker` process), `_capabilities.py` (authoritative server-side `CapabilityIndex` populated by worker heartbeats, and the pydantic `Capability` wire model that validates charset / length at the boundary), langgraph config generation (`_langgraph_config.py`), `server_bootstrap.py` (0-arg Aegra wrappers for `build_chat_graph`, `build_default_graph`, `build_execution_graph`; `bootstrap_server()` idempotent queue factory — call once from lifespan, never from module body; promotes the `monet` logger namespace to INFO at boot — respecting `MONET_LOG_LEVEL` override). `_auth.py` — `require_api_key` is a no-op (open endpoint) when `MONET_API_KEY` is unset (keyless dev mode); strict Bearer check when key is configured. `require_task_auth` always requires a key (HMAC derivation). `_routes.py` exposes the unified `POST /api/v1/workers/{worker_id}/heartbeat` (registration = first heartbeat), `POST /api/v1/pools/{pool}/claim` (pool-scoped claim auth — worker must be heartbeating for that pool), `GET /api/v1/agents` (auth-gated when key configured, open in keyless dev; reads the `CapabilityIndex`), plus `GET /api/v1/artifacts/{id}` / `{id}/view`.
...
**Critical:** Aegra re-executes graph modules via `importlib.util.spec_from_file_location` under a synthetic `aegra_graphs.*` namespace — the module body runs again in a fresh namespace. `server_bootstrap.py` must never create process-singleton state (queues, global connections) at module body level. All such wiring lives in `bootstrap_server()`, called once from `_aegra_routes._lifespan`. This prevents split-brain `InMemoryTaskQueue` where the monolith worker polled a different instance than `invoke_agent` enqueued into.
