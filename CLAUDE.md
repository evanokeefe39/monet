...
- `src/monet/` — package source (src layout)
  ...
  - `orchestration/` — flat-DAG subgraphs and the compound default graph. Public surface: `RunState` (slim parent state for the compound graph, designed for `MyRunState(RunState)` extension), `build_entry_subgraph` / `build_planning_subgraph` / `build_execution_subgraph` (uncompiled `StateGraph`s for users composing their own pipelines), `build_default_graph` (composes the three subgraphs as nodes under `RunState`), `build_chat_graph`. Private state schemas and `RoutingSkeleton`/`RoutingNode` pydantic models in `_state.py`; `_invoke.py` (queue-only dispatch), `_signal_router.py`, `_validate.py`.
  - `server/` — `bootstrap()` with guaranteed ordering (`_bootstrap.py`), lazy-worker wiring, Aegra custom HTTP routes (`_aegra_routes.py`), langgraph config generation (`_langgraph_config.py`), `default_graphs.py` (0-arg Aegra wrappers for `build_chat_graph` and `build_default_graph`).
...
- `tests/` — pytest test directory. `_fakes.py` centralizes the fake LangGraph SDK client. `test_default_compound_graph.py` exercises the post-Track-B compound graph (simple-triage short-circuit, planning-interrupt pause, approve-and-drive, reject-halts-pipeline). `test_subgraph_composition_spike.py` pins the LangGraph properties the collapse depends on. `test_public_api.py` pins the public surface of `monet`, `monet.client`, `monet.orchestration`. `tests/e2e/` holds opt-in end-to-end tests behind the `e2e` pytest marker (set `MONET_E2E=1` to run).
