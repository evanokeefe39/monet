...
- `src/monet/` — package source (src layout)
  ...
  - `orchestration/` — flat-DAG subgraphs and the compound default graph. Public surface: `RunState` (slim parent state for the compound graph, designed for `MyRunState(RunState)` extension), `build_planning_subgraph` / `build_execution_subgraph` (uncompiled `StateGraph`s for users composing their own pipelines), `build_default_graph` (composes planning + execution as nodes under `RunState`), `build_chat_graph` (slash-parse → binary triage `{chat, plan}` → planner / questionnaire / approval three-node state machine, or direct-LLM respond, or slash-invoked specialist). Triage is information-only — returns route + confidence; it does not pick an agent. `planner_node` makes one agent call per visit and writes either `last_plan_output` or `pending_questions` into `ChatState`; routing is state-based so durable execution survives crashes. `questionnaire_node` renders `pending_questions` via the `select_or_text` form field, bumps `followup_attempts`, injects answers back into the next planner context. `approval_node` interrupts with approve/revise/reject — revise writes `plan_feedback` and routes back to planner. Bounds: `PLAN_MAX_REVISIONS=3`, `MAX_FOLLOWUP_ATTEMPTS=1` (the agent is force-planned on the next call when exceeded). The reference planner agent emits either `{kind:"plan"}` with a WorkBrief or `{kind:"questions"}` + signal `NEEDS_CLARIFICATION`; orchestrator routes on the signal, not prose. Private state schemas and `RoutingSkeleton`/`RoutingNode` pydantic models in `_state.py`; `_invoke.py` (queue-only dispatch), `_signal_router.py`, `_validate.py`.
  - `server/` — `bootstrap()` with guaranteed ordering (`_bootstrap.py`), lazy-worker wiring, Aegra custom HTTP routes (`_aegra_routes.py`), langgraph config generation (`_langgraph_config.py`), `default_graphs.py` (0-arg Aegra wrappers for `build_chat_graph` and `build_default_graph`; promotes the `monet` logger namespace to INFO at boot — respecting `MONET_LOG_LEVEL` override — so `invoke_agent` dispatch, queue enqueue/claim/complete, worker execute, and server worker.register/heartbeat records surface through Aegra's structlog pipeline). `_routes.py` adds a `GET /api/v1/artifacts/{id}` (raw bytes) and `GET /api/v1/artifacts/{id}/view` (self-contained HTML page; WorkBrief JSON renders as goal + dependency-annotated steps + assumptions, other JSON pretty-prints, binary falls back to a size notice) — the chat TUI deep-links to `/view` so `ctrl+click` in the transcript opens the rendered plan.
...
- **CLI surface**: `monet dev` (group: default=start, `monet dev down` for teardown), `monet run` (default pipeline vs single-graph via `--graph <entrypoint>`), `monet runs` (list/inspect/pending/resume), `monet chat` (Textual TUI: `RichLog` transcript, slash-command ghost-text via `RegistrySuggester`, `CommandPalette` provider, `LoadingIndicator` spinner during busy turns, `/runs` command for pipeline-run history, `InterruptScreen` modal walking the `Form`/`Field` schema with `render: "inline"|"modal"` hint — inline forms mount above the prompt via `InlineInterruptForm`. Field types include `select_or_text` for claude-code-style questionnaires where a pick-list has a built-in "write your own" path.), `monet worker`, `monet register`, `monet server`, `monet status`.
