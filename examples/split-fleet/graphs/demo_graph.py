...
topic input. The two agents run on different pools, so different worker
fleets claim them. The graph demonstrates pool-routed dispatch end-to-
end when driven with ``monet run --graph demo "<topic>"``.
"""
...
async def _run_fast(state: DemoState) -> dict[str, Any]:
    result = await invoke_agent("fast_agent", "fast", task=state.get("topic", ""))
    return {"fast_result": str(result.output or "")}


async def _run_heavy(state: DemoState) -> dict[str, Any]:
    result = await invoke_agent("heavy_agent", "fast", task=state.get("topic", ""))
