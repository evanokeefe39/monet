...
If no docstring is present, the decorator emits a warning at decoration time. This is a jidoka signal — a missing description is a quality problem worth surfacing immediately rather than discovering at runtime when the planner has insufficient information to make good routing decisions. The warning does not prevent registration.
...
async def researcher(task: str):
    """Quick lookup for a bounded topic. Returns an inline result."""
...
async def rust_analyst(task: str):
    """Fast bounded analysis delegated to the Rust analytics service."""
...
**Command descriptions and the planner**: the planner is typically the first agent invoked in a workflow and is responsible for producing a structured work brief that specifies which agents and commands downstream agents should invoke. To reason correctly about this, the planner needs to know what each registered command actually does. Command descriptions captured from docstrings at decoration time are loaded into the planner's skill context at invocation time, giving it a self-updating view of available capabilities. Adding a new agent with well-written command docstrings makes it available to the planner without any graph changes, as long as a graph path exists to invoke that agent. The quality of the planner's routing decisions is directly proportional to the quality of command docstrings — this is why the decoration-time warning for missing docstrings matters.
...
Conditional edges read confidence, completeness, and `semantic_error` signals from state. `revision_notes` from the output envelope flow back into the next agent invocation's context list as a typed `instruction` entry. Maximum iteration counts are set as conditional edge exit conditions. The three-graph topology section above describes how reflection is structured at the wave level via post-wave QA, at the planning level via the human approval gate, and across runs via the kaizen hook.
...
### Supervisor Graph Topology — Toyota Way Applied

The system is structured as three graphs with clean handoffs between them. This separation follows Toyota Way Principle 2 (continuous flow — each graph has a single, focused responsibility) and makes each graph independently testable, independently restartable from its own checkpoint, and independently deployable.

**Why three graphs and not one**

A single monolithic graph carrying planning, execution, and synthesis in one StateGraph produces a large, ambiguous state object with fields relevant to only one phase at a time. It also conflates two fundamentally different modes of operation — iterative exploration (planning) and structured execution (production) — into a single topology, making both harder to reason about and harder to test. The three-graph structure keeps concerns separated at the graph level, applying the blackbox principle to graphs as it is applied to agents.

#### Entry Point — Triage

Every user message enters the system through a lightweight triage node. The triage node calls the planner in `fast` mode to classify the request and return a structured triage decision — complexity level, which agents are likely needed, and whether full planning is required. This is a single bounded agent call, not an LLM reasoning step in the supervisor.
...
Simple requests receive a direct response without invoking any agent. Bounded requests invoke a single appropriate agent in `fast` mode without a work brief. Complex requests enter the planning graph. The triage classification prevents the planning and execution overhead from being applied to every message regardless of actual complexity — this is Toyota Way Principle 3, pull not push: work is pulled into the heavier process only when it is genuinely needed.

#### Planning Graph

The planning graph handles iterative plan construction with agent assistance and the human approval gate. Planning is exploratory — the planner may need research or analysis to inform the plan before it can be committed to. The graph provides the agents the planner needs via pull, not by running all possible research speculatively (Principle 3, eliminate overproduction).
...
[human approval interrupt] ◄── nemawashi gate
  │                             (Principle 13: slow consensus,
  │                              then fast execution)
  ├── approved ──────────────► END → execution graph
  │
  ├── revise ────────────────► [planner with feedback as instruction entry]
  │                            (bounded revision count)
  │
  └── reject ────────────────► END → kaizen hook (record why)
```

The planner signals which agents it needs via structured output — not by calling agents directly. The graph routes to those agents and feeds their output back to the planner as typed `artifact` context entries. The planner remains a blackbox; the graph does the coordination. This is Principle 5 (jidoka) applied to planning — the planner stops and signals rather than proceeding on incomplete information.

The human approval interrupt is the nemawashi gate (Principle 13). The human sees the full plan: phases, dependency waves, agent assignments, quality criteria, and the planner's explicitly stated assumptions. Approval means genuine agreement, not rubber-stamping. If the human and planner cannot converge within a configured maximum revision count, the process terminates with an escalation rather than either party capitulating.

The approved plan is written to the catalogue as a structured artifact. Its URL is the handoff to the execution graph.

**Plan structure — phases and dependency waves**

The plan decomposes work into phases. Each phase contains dependency waves — groups of agent tasks that have no dependencies on each other within the wave and can execute in parallel, but must complete before the next wave begins. This structure is the direct implementation of Principle 2 (continuous flow) — the smallest batch that can flow without blocking.
...
The execution graph is a faithful executor of the approved plan. It does not make planning decisions — if something unexpected happens it interrupts for human input rather than autonomously replanning. Autonomous replanning is a planning graph concern.
...
[execute wave] ──► LangGraph Send API ──► parallel agent invocations
  ...
  │ wave complete
  ▼
[post-wave reflection: QA/fast]  ← jidoka checkpoint (Principle 5)
...
**Post-wave reflection** is jidoka at the wave level (Principle 5 — build quality in, stop and signal when a defect is detected). After every wave completes, before the next wave starts, the QA agent in `fast` mode receives the wave's expected outputs from the plan and the actual outputs produced, and returns a structured quality assessment. Downstream agents never receive defective inputs from a preceding wave — the reflection gate enforces this. The reflection is not skippable by agent signals.

**The andon board** (Principle 7 — visual control). The node wrapper calls `emit_progress()` automatically at the start and end of each agent invocation, emitting a human-readable status event that the chat layer renders as a running log. The human can see the system's actual state at any point during execution: which phase, which wave, which agents are running, which have completed, and what QA said about each wave. This is the visual control made real — not just a trace in Langfuse but a live feed to the user.

**Error handling escalation ladder** consistent across all waves:

`needs_human_review` always interrupts. The human sees full context — phase, wave, agent, output, reason — and chooses to approve and continue, provide corrective instructions and retry, or abort execution.

`escalation_requested` interrupts only if the human has permissions or context required to resolve it. Auto-resolvable escalations are handled by policy without interruption.

`semantic_error` triggers retry up to the command's configured maximum. Retries exhausted routes to human review. Unrecoverable semantic error at a blocking dependency terminates the phase and routes to the kaizen hook with a partial completion record.

The execution graph never makes autonomous decisions about error recovery. It interrupts and waits. This is the autonomy spectrum position for unexpected failures — humans in the loop.

#### Kaizen Hook

The kaizen hook fires unconditionally after every execution — successful, partial, or failed. A failed execution is more valuable for learning than a successful one if the failure is well-documented. This is Principle 14 (hansei and kaizen) — honest reflection, then continuous improvement.
...
├── record: agent confidence distributions from Langfuse
├── write: hansei record to kaizen log (catalogue artifact)
└── return: final message to user
```

**Final synthesis** is the honest comparison of what was planned against what was actually produced. It notes deviations explicitly — what was descoped, what took longer than planned, what required human intervention and why. This is not a status report; it is a learning document.

**The hansei record** accumulates across runs in the kaizen log. Over time patterns in the log inform improvements to agent prompts, skill files, quality criteria, and SLA characteristics in capability descriptors. The confidence calibration query from Spike 4 feeds into this — systematic miscalibration detected in Langfuse becomes a kaizen action item. This is the operational kaizen loop: observability data → hansei record → descriptor updates → better agent behaviour.

#### Toyota Way Principles — Structural Mapping

| Principle | Structural expression |
|---|---|
| P2 — Continuous flow | Wave-based execution: smallest parallel batch that can flow without blocking |
| P3 — Pull systems | Triage pulls complexity into heavier process only when needed. Planner pulls only the research it actually needs. Waves pull agent work in dependency order |
| P5 — Jidoka | Post-wave reflection gate. Agent signal system. Plan approval interrupt. Publisher never fires without QA pass |
| P6 — Standardised tasks | Uniform input/output envelopes across all agents. Node wrapper as the standardisation enforcement point |
| P7 — Visual control | Node wrapper `emit_progress()` as andon board. Langfuse trace as execution record. Hansei record as written reflection |
| P12 — Genchi genbutsu | Planner pulls actual research before committing to plan. Post-wave reflection reads actual outputs, not self-reported confidence |
| P13 — Nemawashi | Human approval gate before execution. Bounded revision count — forced consensus is not consensus |
| P14 — Hansei and kaizen | Kaizen hook fires unconditionally. Hansei record accumulates. Confidence calibration data pipeline feeds continuous improvement |
...
| Base agent definitions | Base system prompt, toolset, default extension stack per command, and SLA metadata for each of the five reference agents |
...
The three-graph topology described in the Supervisor Graph Topology section above is the primary structural expression of the Toyota Way principles in this architecture. The mapping is summarised in the table at the end of that section. This section covers the alignment and future application dimensions for each relevant principle.
...
Current alignment: Wave-based execution in the execution graph is the direct implementation. Each wave is the smallest batch of work that can flow without blocking. Items within a wave execute in parallel. Items across waves respect dependency order. The post-wave reflection gate surfaces problems at the earliest possible point rather than allowing defects to accumulate and be discovered at the end.

Future application: The most significant flow interruption in the current design is the planning phase approval checkpoint. The human must approve the work brief before production begins. This is correct from a quality perspective (nemawashi) but it is a batch point in the flow. A future iteration might explore progressive brief refinement — production begins on settled phases of the brief while unsettled phases continue to be refined — reducing the gap between planning and execution without sacrificing quality.

**Principle 3: Pull not push**

Current alignment: Triage pulls complexity into the heavier planning and execution process only when the request genuinely needs it. The planner pulls only the research it actually needs to produce the plan — it signals which specific agent to call with which specific question, not a speculative broad sweep. Wave-based execution pulls the next wave only when the preceding wave has passed reflection. No agent pre-computes output speculatively.
...
Current alignment: Post-wave reflection is jidoka at the wave level — production stops and surfaces the problem before defective outputs are passed to downstream agents. Write-time invariants in the artifact catalogue reject malformed artifacts at the boundary. The node wrapper validates output envelopes before updating graph state. The QA agent is a structural safeguard, not an optional step. The `@agent` decorator catches unexpected exceptions before they crash the node. The plan approval interrupt prevents execution on an unapproved brief. Each of these is a quality gate built into the system at the appropriate level.

Future application: The weakest jidoka point remains confidence score reliability. The kaizen hook's confidence calibration data pipeline — comparing agent-declared confidence against actual QA outcomes over time via Langfuse — is the path to improving this. A miscalibrated agent that consistently declares high confidence on outputs that QA rejects is a detectable defect in the quality gate. Detection is the first step; the kaizen loop is what closes it.
...
Current alignment: The node wrapper calls `emit_progress()` at the start and end of every agent invocation, providing a live running log to the chat layer — the andon board for the execution graph. Langfuse surfaces the full OTel trace. Pi session logs are the ground truth for agent internals. The hansei record makes the comparison of plan versus actuality visible after every run.

Future application: The gap is a unified operational view across OTel traces in Langfuse, graph state in the Postgres checkpointer, and artifacts in the catalogue — for a single run. Today these require separate queries. A future operational dashboard that joins these views by run ID would significantly reduce the time to diagnose unexpected behaviour. This is a tooling gap, not an architectural one, and can be built on top of the existing stores without changing any layer.

**Principle 8: Proven technology**

Current alignment: Python `contextvars` for the SDK context system — a standard library module since Python 3.7. LangGraph for orchestration, backed by LangChain's production track record. SQLAlchemy for the index. Langfuse for observability, self-hostable and OTel-native. Pi-agent as the reference runtime, minimal and transparent.

Future application: The most likely pressure for unproven technology adoption is in the LLM layer. New models, new providers, new reasoning architectures will appear regularly. The agent capability descriptor's per-command model hints and the agent's internal model selection provide the right mechanism — adopt new models at the agent level via configuration, not through architectural change. The system should be suspicious of any new technology adoption that requires changes to more than one layer.

**Principle 12: Genchi genbutsu**

Current alignment: The planner pulls actual research before committing to a plan rather than speculating. The post-wave reflection reads actual outputs against the plan's expected outputs — not self-reported confidence. The kaizen hook compares actual execution against the planned phases and waves. Every agent session is a JSONL file. Every artifact is in the catalogue with full provenance. Every graph execution is in the Postgres checkpointer. When something goes wrong, the full picture is available without asking anyone.

Future application: The genchi genbutsu principle argues against summary-only reporting as the primary operational interface. When a specific run behaves unexpectedly, the right response is to read the pi session log and the wave reflection records, not to ask Langfuse what the average confidence was. The operational practice should reinforce this — anomalies are investigated at the source, not at the aggregate.

**Principle 13: Nemawashi**

Current alignment: The planning graph embodies nemawashi. The planner iterates with the researcher and the human until the work brief represents genuine consensus on the goal, scope, quality criteria, phases, and dependency waves. The bounded revision count enforces that forced consensus is not consensus — if agreement cannot be reached, the process terminates rather than proceeding on an unapproved plan. Nothing expensive begins until that consensus is reached. The execution phase is then fast and largely deterministic because the decision-making was done upfront.

Future application: Nemawashi applies beyond the planning phase. Significant changes to the system architecture — adding a new agent, changing the artifact schema, updating a skill file that affects multiple agents — should go through a lightweight review process before being implemented. The architecture document itself is a nemawashi artifact.

**Principle 14: Hansei and kaizen**

Current alignment: The kaizen hook fires unconditionally after every execution — successful, partial, or failed. It writes a hansei record comparing planned against actual execution, notes deviations, and emits confidence calibration data. The hansei record accumulates across runs in the kaizen log. The observability layer is designed to accumulate the data that enables kaizen — Langfuse actuals against declared SLA characteristics, QA revision notes surfacing recurring failure patterns, confidence calibration data.

Future application: The data infrastructure is necessary but not sufficient. Kaizen requires a defined cadence for reviewing the data and a process for translating findings into improvements. How often are agent system prompts reviewed against QA outcomes? Who owns skill file updates? When does a pattern of low-confidence outputs trigger a prompt revision rather than just a retry? These are process questions that need answers before kaizen becomes real rather than aspirational. The architecture enables kaizen. The operating model delivers it.
